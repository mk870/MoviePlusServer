...
import restapi.app.JwtFilter.JwtUtil;
import restapi.app.Repositories.UserRepository;
import restapi.app.Repositories.VerificationTokenRepository;
...
public class UserServiceImpl implements UserService{
 ...
 public ResponseDTO validateToken(String token) {
   ...
   if(verificationToken == null){
     return new ResponseDTO(null, "invalid");
   }
   ...
   if((verificationToken.getExpirationTime().getTime() - cal.getTime().getTime())<= 0){
     ...
     return new ResponseDTO(null, "Your verification period has expired please signup again");
   }
   //else if all is well
   ...
   return new ResponseDTO(jwt, "verification successful");
 }
 @Override
 public ResponseDTO userLogin(LoginDTO loginDTO) {
   try {
     ...
     return new ResponseDTO(null,"Incorrect Credentials");
   }
   final UserDetails userDetails = userDetailsServiceImpl.loadUserByUsername(loginDTO.getEmail());
   final String jwt = jwtUtil.generateToken(userDetails);
   
   return new ResponseDTO(jwt,"login successful");
